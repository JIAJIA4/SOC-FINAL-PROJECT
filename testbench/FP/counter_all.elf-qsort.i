...
int* __attribute__ ( ( section ( ".mprjram" ) ) ) qsort(){
 ...
 int ans;
 (*(volatile uint32_t*)0x30002000) = 1;

 for(int i = 0; i < 10; i++){
  while(!(((*(volatile uint32_t*)0x30002000) >> 4) & 1));
  (*(volatile uint32_t*)0x30002080) = Q[i];
 }

 for(int i = 0; i < 10; i++){
  while(!(((*(volatile uint32_t*)0x30002000) >> 5) & 1));

  Q[i] = (*(volatile uint32_t*)0x30002084);
 }
